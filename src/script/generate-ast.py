...
"""
Generate ast implementation from a spec that lists statement and expression AST nodes and their fields.

For an example, see src/script/ast-spec.json.

Usage:
  python src/script/generate_ast.py --output src/ast.cppm --spec src/script/ast-spec.json
...
def render_forward_declarations(categories: List[NodeCategory]) -> str:
  ...
  for index, category in enumerate(categories):
    lines.append(f"export namespace {category.namespace} {{")
...
def render_void_visitors(categories: List[NodeCategory]) -> str:
  ...
  for index, category in enumerate(categories):
    ...
    lines.append(f"export class {category.label}VisitorVoid {{")
    lines.append("  public:")
...
def render_type_erased_visitors(categories: List[NodeCategory]) -> str:
  ...
  for index, category in enumerate(categories):
    param_id = "stmt" if category.label == "Stmt" else "expr"
    lines.append(f"export class {category.label}VisitorAny {{")
    lines.append("  public:")
    for node in category.types:
      suffix = visitor_method_suffix(node, category)
...
def render_concrete_visitors(categories: List[NodeCategory]) -> str:
  ...
  for index, category in enumerate(categories):
    param_id = "stmt" if category.label == "Stmt" else "expr"
    lines.append("export template <typename R>")
...
def render_node_class(category: NodeCategory, node: NodeType) -> str:
  ...
  lines = [
    f"export class {category.namespace}::{node.name} : public {category.label} {{",
    "  public:",
  ]
...
def main() -> None:
  parser = argparse.ArgumentParser(description="Generate ast module from a JSON spec.")
  parser.add_argument("--spec", type=Path, help="Path to JSON specification file.")
  parser.add_argument("--output", type=Path, default=Path("src/ast.cppm"), help="Where to put the ast implementation.")
