...
public:
  ...
  virtual std::any visit_print_expr_any(std::shared_ptr<Print> expr) = 0;
...
public:
  ...
  virtual R visit_print_expr(std::shared_ptr<Print> expr) = 0;
...
private:
  ...
  std::any visit_print_expr_any(std::shared_ptr<Print> expr) final {
...
public:
  Binary(const Token& op, NamedFunction fn_name, Expression left, Expression right) : op {op}, fn_name {fn_name}, left {std::move(left)}, right {std::move(right)} {}
...
public:
  Unary(const Token& op, NamedFunction fn_name, Expression expr) : op {op}, fn_name {fn_name}, expr {std::move(expr)} {}
...
public:
  explicit Grouping(Expression expr) : expr {std::move(expr)} {}
...
class Print : public Expr, public std::enable_shared_from_this<Print> {
public:
  Print(const Token& op, Expression expr) : op {op}, expr {std::move(expr)} {}
