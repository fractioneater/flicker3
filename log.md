...
I got CMake hooked up with Google Test, ready to get going. There was little to test, so it wasn't as satisfying as actual language design.
...
mainly for the quality (helpfulness) of error messages. That's a priority for me.
...
And, what's more, the log is now up to date. I am actually writing this on the day it is listed as, unlike the past five entries.
...
name of this function so it can be resolved then called. There are still some iffy things about this—plot holes, you could say—but it makes the DOT printing
much easier.

And, because it has more benefits than drawbacks, `print` is now an expression. `error` is too.

The file `ast.h` is now generated by a Python script. It has statement support as well! That means the next time I get a chance to work on this, Flicker
programs
...
they're really not worth it if all they do is create an if-elif-elif-elif-else AST internally. If they're special (jump tables), they should have
...
Variable declarations are also implemented, at least as far as the parser will see them go.
...
- Loop frames for labels (break and continue)
